...
export default class App extends Component {  
    ...
    return (
      ...
      <Typography variant="h6">Your order is:  {
        order.length == 0 ? (
          ...
          order.map(item => (
            <Chip onDelete={() => this.delFromShoppingCart(item)} label={item.pizza.name + " x" + item.qt} color="primary"/>
          ))
        )
  ...
  displayPrice = (row) => row.price
  //displayImage = (row) => row.image
  handleConfirm(e) {
    e.preventDefault()
    fetch("/api/confirm")
    .then(res => res.json())
              ...
              <TableBody>
                {rows.map(row => (
                  <TableRow key={row.name}>
                    ...
                    <TableCell align="right"><span>{this.displayPrice(row)}</span></TableCell>
